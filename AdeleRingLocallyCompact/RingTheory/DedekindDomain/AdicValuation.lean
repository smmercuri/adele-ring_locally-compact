...
theorem isUnit_of_valuation_eq_one {x : v.adicCompletionIntegers K}
  ...
  have hx_unit : IsUnit (x : v.adicCompletion K) := by
    simp only [isUnit_iff_ne_zero, ne_zero_iff_valuation_ne_zero, hx, ne_eq, one_ne_zero,
      not_false_eq_true]
  obtain ⟨u, hu⟩ := hx_unit
...
theorem eq_pow_uniformizer_mul_unit {x π : v.adicCompletionIntegers K} (hx : x.val ≠ 0)
  ...
  have hm₀ : 0 ≤ m := by
    rw [Right.nonneg_neg_iff, ← toAdd_one, Multiplicative.toAdd_le, ← WithZero.coe_le_coe,
      WithZero.coe_unzero]
    exact x.property
  use Int.toNat m
  have hpow : Valued.v (π ^ (-m) * x.val) = 1 := by
    rw [Valued.v.map_mul, map_zpow₀, hπ, ofAdd_neg, WithZero.coe_inv, inv_zpow', neg_neg,
      ...
      WithZero.coe_unzero, inv_mul_cancel hx₀]
...
theorem Ideal.Quotient.mk_out' {R : Type*} [CommRing R] {I : Ideal R} {x : R ⧸ I} :
...
theorem finiteExpansion {π : v.adicCompletionIntegers K} (n : ℕ) (x : v.adicCompletionIntegers K)
  ...
  | succ d hd =>
    ...
    simp only [Fin.snoc_last, List.concat_eq_append,
    ...
    rw [← @sub_sub _ (inferInstanceAs (SubtractionCommMonoid (v.adicCompletionIntegers K))),
      ← hz, ← sub_mul, Ideal.mem_span_singleton, pow_succ]
...
/-- There is a finite covering of the `v`-adic integers of open balls of radius equal to one,
...
theorem isTotallyBounded :
  ...
  · exact hasFiniteSubcover_of_openBall_one_lt K v hγ
  by_cases hγ' : (γ : WithZero (Multiplicative ℤ)) = 1
  · exact hasFiniteSubcover_of_openBall_eq_one K v hγ'
...
instance : LocallyCompactSpace (v.adicCompletionIntegers K) := by
  haveI : WeaklyLocallyCompactSpace (v.adicCompletionIntegers K) :=
    instWeaklyLocallyCompactSpaceOfCompactSpace
  haveI : RegularSpace (v.adicCompletionIntegers K) :=
    TopologicalAddGroup.regularSpace _
  haveI : R1Space (v.adicCompletionIntegers K) :=
    instR1Space
...
theorem openBall_isCompact (γ : (WithZero (Multiplicative ℤ))ˣ) : IsCompact (openBall K v γ) := by
  ...
  instance locallyCompactSpace : LocallyCompactSpace (v.adicCompletion K) := by
    ...
    have h_image_f : f '' (openBall K v γ) = setOf (λ y => Valued.v (y - x) < γ) := by
      ...
      simp only [f]
      refine Iff.intro
        (fun ⟨a, ha, hay⟩ => by rwa [← hay, Set.mem_setOf_eq, add_sub_assoc, sub_self, add_zero])
        (fun hy => by use (y - x); exact ⟨hy, by simp only [sub_add_cancel]⟩)
