...
lemma adicCompletion_exists_uniformizer :
  ...
  obtain ⟨x, hx⟩ := HeightOneSpectrum.valuation_exists_uniformizer K v
  use ↑x
  rw [HeightOneSpectrum.valuedAdicCompletion_def, ← hx]
  simp
  rfl
...
lemma adicCompletionIntegers_exists_uniformizer :
  ...
  refine' ⟨⟨x, _⟩, _⟩
  {
    ...
    rw [HeightOneSpectrum.valuedAdicCompletion_def]
    simp
